...
for (i in (1:length(Patient_ID))){
  ...
  DiasBP.data = matrix(nrow = 1,ncol = 25)
  idx2 = (Patient.data$Time<=Start.time)
  Hourly.data = Patient.data[idx2,"DiasBP"]
  #Remove outliers
  Hourly.data = Hourly.data[(Hourly.data<=150) & (Hourly.data>=40)]
  if (length(Hourly.data) == 0){Hourly.data=0}
  DiasBP.data[1] = as.numeric(round(mean(Hourly.data),digits = 2))
  ...
  for (j in 1:24){
    ...
    #Remove outliers
    Hourly.data = Hourly.data[(Hourly.data<=150) & (Hourly.data>=40)]
    Mean.DiasBP = round(mean(Hourly.data), digits = 2)
    
    if (length(Hourly.data)==0){
      #If no data is available within the hour, use previous reading
      DiasBP.data[j+1] = DiasBP.data[j]
    } else {
      DiasBP.data[j+1] = Mean.DiasBP
    }
    #Remove used data
    Patient.data = Patient.data[!idx2,]
  }
  Filtered.data[i,2:26] = DiasBP.data
  if(i %% 1000 == 0) print(i)
}
...
write.csv(Filtered.data,file = "DiasBP_Filtered.csv")
save(Filtered.data,file = "DiasBP_Filtered.RData")
...
write.csv(Filtered.data2,file = "DiasBP_Filtered_NOzero.csv")
