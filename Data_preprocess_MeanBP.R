...
for (i in (1:length(Patient_ID))){
  ...
  MeanBP.data = matrix(nrow = 1,ncol = 25)
  idx2 = (Patient.data$Time<=Start.time)
  Hourly.data = Patient.data[idx2,"MeanBP"]
  #Remove outliers
  Hourly.data = Hourly.data[(Hourly.data<=250) & (Hourly.data>=40)]
  if (length(Hourly.data) == 0){Hourly.data=0}
  MeanBP.data[1] = as.numeric(round(mean(Hourly.data),digits = 2))
  ...
  for (j in 1:24){
    ...
    #Remove outliers
    Hourly.data = Hourly.data[(Hourly.data<=250) & (Hourly.data>=40)]
    Mean.MeanBP = round(mean(Hourly.data), digits = 2)
    
    if (length(Hourly.data)==0){
      #If no data is available within the hour, use previous reading
      MeanBP.data[j+1] = MeanBP.data[j]
    } else {
      MeanBP.data[j+1] = Mean.MeanBP
    }
    #Remove used data
    Patient.data = Patient.data[!idx2,]
  }
  Filtered.data[i,2:26] = MeanBP.data
  if(i %% 1000 == 0) print(i)
}
...
write.csv(Filtered.data,file = "MeanBP_Filtered.csv")
save(Filtered.data,file = "MeanBP_Filtered.RData")
...
write.csv(Filtered.data2,file = "MeanBP_Filtered_NOzero.csv")
