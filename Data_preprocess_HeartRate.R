...
for (i in (1:length(Patient_ID))){
  ...
  HeartRate.data = matrix(nrow = 1,ncol = 25)
  idx2 = (Patient.data$Time<=Start.time)
  Hourly.data = Patient.data[idx2,"Valuenum"]
  #Remove outliers
  Hourly.data = Hourly.data[(Hourly.data<=200) & (Hourly.data>=30)]
  if (length(Hourly.data) == 0){Hourly.data=0}
  HeartRate.data[1] = as.numeric(round(mean(Hourly.data),digits = 2))
  ...
  for (j in 1:24){
    ...
    #Remove outliers
    Hourly.data = Hourly.data[(Hourly.data<=200) & (Hourly.data>=30)]
    Mean.HeartRate = round(mean(Hourly.data), digits = 2)
    
    if (length(Hourly.data)==0){
      #If no data is available within the hour, use previous reading
      HeartRate.data[j+1] = HeartRate.data[j]
    } else {
      HeartRate.data[j+1] = Mean.HeartRate
    }
    #Remove used data
    Patient.data = Patient.data[!idx2,]
  }
  Filtered.data[i,2:26] = HeartRate.data
  if(i %% 1000 == 0) print(i)
}
...
write.csv(Filtered.data,file = "HeartRate_Filtered.csv")
save(Filtered.data,file = "HeartRate_Filtered.RData")
...
write.csv(Filtered.data2,file = "HeartRate_Filtered_NOzero.csv")
