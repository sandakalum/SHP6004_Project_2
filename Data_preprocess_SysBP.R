...
for (i in (1:length(Patient_ID))){
  ...
  SysBP.data = matrix(nrow = 1,ncol = 25)
  idx2 = (Patient.data$Time<=Start.time)
  Hourly.data = Patient.data[idx2,"SysBP"]
  #Remove outliers
  Hourly.data = Hourly.data[(Hourly.data<=250) & (Hourly.data>=60)]
  if (length(Hourly.data) == 0){Hourly.data=0}
  SysBP.data[1] = as.numeric(round(mean(Hourly.data),digits = 2))
  ...
  for (j in 1:24){
    ...
    #Remove outliers
    Hourly.data = Hourly.data[(Hourly.data<=250) & (Hourly.data>=60)]
    Mean.SysBP = round(mean(Hourly.data), digits = 2)
    
    if (length(Hourly.data)==0){
      #If no data is available within the hour, use previous reading
      SysBP.data[j+1] = SysBP.data[j]
    } else {
      SysBP.data[j+1] = Mean.SysBP
    }
    #Remove used data
    Patient.data = Patient.data[!idx2,]
  }
  Filtered.data[i,2:26] = SysBP.data
  if(i %% 1000 == 0) print(i)
}
...
write.csv(Filtered.data,file = "SysBP_Filtered.csv")
save(Filtered.data,file = "SysBP_Filtered.RData")
...
write.csv(Filtered.data2,file = "SysBP_Filtered_NOzero.csv")
