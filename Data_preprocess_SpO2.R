...
for (i in (1:length(Patient_ID))){
  ...
  SpO2.data = matrix(nrow = 1,ncol = 25)
  idx2 = (Patient.data$Time<=Start.time)
  Hourly.data = Patient.data[idx2,"SpO2"]
  #Remove outliers
  Hourly.data = Hourly.data[(Hourly.data<=100) & (Hourly.data>=60)]
  if (length(Hourly.data) == 0){Hourly.data=0}
  SpO2.data[1] = as.numeric(round(mean(Hourly.data),digits = 2))
  ...
  for (j in 1:24){
    ...
    #Remove outliers
    Hourly.data = Hourly.data[(Hourly.data<=100) & (Hourly.data>=60)]
    Mean.SpO2 = round(mean(Hourly.data), digits = 2)
    
    if (length(Hourly.data)==0){
      #If no data is available within the hour, use previous reading
      SpO2.data[j+1] = SpO2.data[j]
    } else {
      SpO2.data[j+1] = Mean.SpO2
    }
    #Remove used data
    Patient.data = Patient.data[!idx2,]
  }
  Filtered.data[i,2:26] = SpO2.data
  if(i %% 1000 == 0) print(i)
}
...
write.csv(Filtered.data,file = "SpO2_Filtered.csv")
save(Filtered.data,file = "SpO2_Filtered.RData")
...
write.csv(Filtered.data2,file = "SpO2_Filtered_NOzero.csv")
